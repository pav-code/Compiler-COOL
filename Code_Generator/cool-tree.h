...
private:
    ...
    bool   no_expr;
public:
    // Environment: class, ID, position
    SymbolTable<Symbol, SymbolTable<Symbol, int> >*Environment;
    SymbolTable<Symbol, int>*FormalEnv;
    //SymbolTable<Symbol, int>*LetEnv;
    //SymbolTable<Symbol, int>*CaseEnv;
    SymbolTable<Symbol, int>*LetEnv;
    SymbolTable<Symbol, int>*CaseEnv;
    //SymbolTable<Symbol, int>*LetCaseEnv;
    SymbolTable<Symbol, std::list<Symbol> >*CaseChildren;
    SymbolTable<Symbol, std::list<int> >*CaseIndexes;
    SymbolTable<Symbol, SymbolTable<Symbol, int> >*Impl;

    SymbolTable<Symbol, int>*ClassTags;
    int labelNum;
    int letPosition;
    int caseEnd;
    Context();
    void   set_current_class(Symbol );
    Symbol get_current_class();
    bool   is_no_expr() { return no_expr; }
    void   set_no_expr(bool );
    int    let_pos() { return (letPosition*(-1)); } /* stack grows towards - */
    void   inc_let_pos() { letPosition++; }
    void   dec_let_pos() { letPosition--; }
    void   set_case_end(int set) { caseEnd = set; }
    int    get_case_end() { return caseEnd; }
};
